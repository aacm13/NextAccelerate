...
const defaultComponents = {
  ...
  label: ({ node, key }) => {
    switch (node.data.label) {
      case 'accent-cyan':
        return (
          <span key={key} className="text-accelerate-cyan font-bold" >
            {node.text}
          </span>
        );
      case 'accent-magenta':
        return (
          <span key={key} className="text-accelerate-magenta font-bold" >
            {node.text}
          </span>
        );
      case 'accent-blue':
        return (
          <span key={key} className="text-accelerate-blue font-bold" >
            {node.text}
          </span>
        );
    }
